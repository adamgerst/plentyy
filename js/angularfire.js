...
(function() {
  ...
  AngularFireAuth.prototype = {
    ...
    login: function(provider, options) {
      console.log("I'm seriously going into AngularFireAuth and failing somewhere else");
    ...
    getCurrentUser: function() {
      console.log("Getting a promise for someone's info - step 2 - NEW ERROR! Yay!");
      var self = this;
      var deferred = this._q.defer();

      if (self._currentUserData !== undefined) {
        console.log("If self._currentuser is undefined");
        deferred.resolve(self._currentUserData);
      } else {
        self._getCurrentUserDeferred.push(deferred);
        console.log("Else self._currentuser is defined");
      }
